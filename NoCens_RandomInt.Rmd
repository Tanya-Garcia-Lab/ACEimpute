...
ee.results = m_estimate(estFUN = eff_score_vec, 
                        ...
                        outer_args = list(response = "Y", invariant.X = "X1", variant.X = "X2"))
...
se.df <- matrix(c(se.theta.lme, se.theta.ee), 2, p, T) %>%
  as.data.frame() %>%
...
```{r Estimate.Parameters.2, cache=T, echo=F}
# use lmer() to fit linear mixed effects model
lme.results = lmer(formula = Y ~ X1 + X2 + (1 | id), data = sample.data) %>%
  summary()
theta.lme = c(coef(lme.results)[,1], lme.results$sigma^2)
...
```{r Estimate.Parameters.3, cache=T, echo=F}
# use lmer() to fit linear mixed effects model
...
ee.results = m_estimate(estFUN = eff_score_vec, 
                        data = sample.data, units = "id",
                        root_control = setup_root_control(start = theta.init),
                        outer_args = list(response = "Y", invariant.X = "X1", variant.X = c("X2", "X3")))
theta.ee = coef(ee.results)
...
```{r Estimate.Parameters.4, cache=T, echo=F}
# use lmer() to fit linear mixed effects model
