...
# (associated with fixed effects, beta) and Z
...
logHR <- c(1, -0.5)
rate.t <- 0.5
rate.c <- 0.3 # rate.c = 0.1 --> ~20% censoring, rate.c = 0.3 --> ~40% censoring
...
for (s in 1:num_sim) {
  ...
  # write.csv(save_res, "~ImputeT_NoInts_MediumCens.csv", row.names = F)
}
